...
func TestLengthPredicates(t *testing.T) {
	...
	cases := []struct {
		...
		// backup key: 0..maxBackupKeyLen
		{"backup_key empty", validBackupKeyLength, "", true},
		{"backup_key max", validBackupKeyLength, rep(maxBackupKeyLen), true},
		{"backup_key max+1", validBackupKeyLength, rep(maxBackupKeyLen + 1), false},
	}
