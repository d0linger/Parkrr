...
func (h *AuthHandler) TOTPEnable(w http.ResponseWriter, r *http.Request) {
	...
	// Issue one-time backup codes (shown once) BEFORE enabling 2FA state.
	// Generating recovery codes prior to executing UPDATE avoids 2FA enablement
	// state inconsistency if backup code generation fails.
