...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if u.TOTPEnabled {
		...
		if lockedFor > 0 {
			slog.Warn("login failed", "user", u.Username, "ip", ip, "reason", "2fa locked", "failures", failures)
		...
		if !ok {
			h.recordLoginFailure(key, ip)
			slog.Warn("login failed", "user", u.Username, "ip", ip, "reason", "bad 2fa code", "failures", failures)
